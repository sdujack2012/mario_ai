...
class DQN:
    def __init__(self, input_shape, continueTraining, name):
        ...
        self.conv1 = image_output = tf.keras.layers.Conv2D(
            filters=32, kernel_size=(3, 3), input_shape=input_shape)(image_input)
        image_output = tf.keras.layers.BatchNormalization(
            trainable=True)(image_output)
        image_output = tf.keras.layers.Activation('elu')(image_output)
        image_output = tf.keras.layers.MaxPooling2D(
            pool_size=(2, 2))(image_output)
        image_output = tf.keras.layers.Dropout(0.25)(image_output)

        self.conv2 = image_output = tf.keras.layers.Conv2D(
            filters=64, kernel_size=(3, 3), activation='elu')(image_output)
        image_output = tf.keras.layers.MaxPooling2D(
            pool_size=(2, 2))(image_output)
        image_output = tf.keras.layers.BatchNormalization(
            trainable=True)(image_output)
        ...
        self.conv3 = image_output = tf.keras.layers.Conv2D(
            filters=128, kernel_size=(3, 3), activation='elu')(image_output)
        image_output = tf.keras.layers.MaxPooling2D(
            pool_size=(2, 2))(image_output)
        image_output = tf.keras.layers.BatchNormalization(
            trainable=True)(image_output)
        ...
        image_output = tf.keras.layers.Dense(
            256, activation='elu')(image_output)

        device_input = tf.keras.layers.Input(shape=(4, ))
        device_output = tf.keras.layers.Dense(
            128, activation='elu')(device_input)
