...
class IO:
        def __init__(self, window_name):
                ...
                self.stacked_frames  =  deque([np.zeros(frame_size) for i in range(stack_size)], maxlen=4) 
        ...
        def action(self, actionIndex):
                if self.pressed_key[actionIndex] == 1:
                        win32api.keybd_event(self.action_mapping[actionIndex], 0 ,win32con.KEYEVENTF_KEYUP ,0)
                else:
        ...
        def get_screenshot(self):   
                ...
                im = im.crop((rect[0]+offset[0], rect[1]+offset[1], rect[2]+offset[0], rect[3]+offset[1])).convert('L')
        ...
        def process_screenshot(self, screenshot):
                ...
                resized_screenshot = croped_screenshot.resize(frame_size, Image.BICUBIC)
                normalized_screenshot = np.asarray(resized_screenshot).reshape(100, 150) / 255.0
        ...
        def get_stacked_frames(self, screenshot, is_new_episode):
                ...
                if is_new_episode:
                        ...
                        self.stacked_frames = deque([np.zeros(frame_size) for i in range(stack_size)], maxlen=stack_size)
